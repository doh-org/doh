...
커밋 메시지에서 극도로 간결하게,
문법보다 간결함을 우선시해라.
대화 답변은 길이를 제한하지 않는다. 말투는 output style `longform-briefing`을 따른다.
코드 주석은 `## 주석 스타일` 규칙을 따른다.
...
배제: 수사·과장("확 달라진다"), 평가 어투("사실상 정상이다"), 정도부사("거의", "사실상"), 감탄, 비유.
문장은 평서 단정형으로 끝낸다.
...
코드에 대한 기계적 설명만 단다. 기획·의도·판단 근거는 쓰지 않는다.
- 언어: 한국어, 한 줄.
