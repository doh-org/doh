...
func setupNaver(t *testing.T) (http.Handler, *testutil.FakeNaver, *testutil.FakeKakao, *testutil.FakeSupabase, *testutil.TestKeys) {
	...
	fk := testutil.NewFakeKakao(t)

	oldSearch, oldGeo, oldKakao := naver.SearchBaseURL, naver.GeocodeBaseURL, kakao.LocalBaseURL
	naver.SearchBaseURL = fn.Server.URL
	naver.GeocodeBaseURL = fn.Server.URL
	kakao.LocalBaseURL = fk.Server.URL
	...
	return router, fn, fk, fs, keys
}

// ── GET /api/v1/places/search: place_controller_test.go 참고 ──────────────
